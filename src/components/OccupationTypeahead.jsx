...
import { searchOccupations } from '../api/core'

export default function OccupationTypeahead({ value, onChange, label = 'Occupation' }) {
  ...
  const handleChange = (e) => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const res = await searchOccupations(text.trim())
        ...
        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-20 max-h-48 overflow-y-auto">
          {suggestions.map((occ) => (
            <button
              key={occ.id}
              type="button"
              onClick={() => selectSuggestion(occ.name)}
